...
public class TypeRoomGUI extends BorderPane {
    ...
    private final TableView<TypeRoom> typeRoomList = new TableView<>(typeRoomItems);
    private final TextField typeRoomIdField = new TextField();
    ...
    private void buildUi() {
        ...
        configureTypeRoomTable();
        typeRoomList.getSelectionModel().setSelectionMode(SelectionMode.SINGLE);

        typeRoomIdField.setEditable(false);
        typeRoomIdField.setPromptText("Tự động");
        nameField.setPromptText("Tên loại phòng");
        ...
        VBox searchBar = new VBox(8, sectionLabel("Danh sách loại phòng"), searchControls);
    ...
    private void configureTypeRoomTable() {
        TableColumn<TypeRoom, Integer> idColumn = new TableColumn<>("ID");
        idColumn.setCellValueFactory(cellData -> new ReadOnlyObjectWrapper<>(cellData.getValue().getTypeRoomId()));
        idColumn.setPrefWidth(90);
        idColumn.setMinWidth(70);

        TableColumn<TypeRoom, String> nameColumn = new TableColumn<>("Name");
