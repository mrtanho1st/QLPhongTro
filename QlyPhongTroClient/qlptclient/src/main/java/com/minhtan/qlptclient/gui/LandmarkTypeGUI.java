...
public class LandmarkTypeGUI extends BorderPane {
    ...
    private final ObservableList<LandmarkType> landmarkTypeItems = FXCollections.observableArrayList();
    private final TableView<LandmarkType> landmarkTypeList = new TableView<>(landmarkTypeItems);
    private final TextField landmarkTypeIdField = new TextField();
    ...
    private void buildUi() {
        ...
        configureLandmarkTypeTable();
        landmarkTypeList.getSelectionModel().setSelectionMode(SelectionMode.SINGLE);

        landmarkTypeIdField.setEditable(false);
        landmarkTypeIdField.setPromptText("Tự động");
        nameField.setPromptText("Tên loại địa điểm");
        ...
        VBox searchBar = new VBox(8, sectionLabel("Danh sách loại địa điểm"), searchControls);
    ...
    private void configureLandmarkTypeTable() {
        TableColumn<LandmarkType, Integer> idColumn = new TableColumn<>("ID");
        idColumn.setCellValueFactory(cellData -> new ReadOnlyObjectWrapper<>(cellData.getValue().getLandmarkTypeId()));
